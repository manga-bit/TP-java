...
public class VisageRond {
    ...
    /**
 * Modifie la valeur du déplacement vertical du visage.
 * @param dy La nouvelle valeur du déplacement vertical.
 */
public void setDy(int dy) {
    this.dy = dy;
}
/**
 * Classe VisageRond qui représente un visage animé.
 */
public class VisageRond {
    private int x, y, rayon;  // Attributs pour les coordonnées et le rayon du visage
    private int dy;           // Attribut représentant le déplacement vertical du visage

    /**
     * Constructeur de la classe VisageRond.
     * @param x Position initiale en abscisse du visage.
     * @param y Position initiale en ordonnée du visage.
     * @param rayon Rayon du visage.
     */
    public VisageRond(int x, int y, int rayon) {
        this.x = x;
        this.y = y;
        this.rayon = rayon;
        this.dy = 0; // Initialisation du déplacement vertical à 0
    }

    // Autres méthodes existantes pour gérer le visage, par exemple pour le dessiner, déplacer, etc.
    ...
    /**
     * Retourne la valeur actuelle du déplacement vertical.
     * @return La valeur de déplacement vertical actuelle.
     */
    public int getDy() {
        return dy;
    }
    ...
    /**
     * Méthode pour déplacer le visage selon le déplacement vertical.
     */
    public void deplacer() {
        this.y += dy;  // Déplace le visage selon dy
    }

    /**
     * Méthode pour dessiner le visage.
     * @param g Objet de la classe Graphics pour dessiner sur l'écran.
     */
    public void dessiner(Graphics g) {
        g.fillOval(x - rayon, y - rayon, rayon * 2, rayon * 2);  // Dessine un cercle (visage)
    }

    // Autres méthodes pour gérer le visage (comme l'animation, la mise à jour des positions, etc.)
}
